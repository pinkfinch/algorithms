...
def can_visit_all_rooms(rooms)
  return true if !rooms || rooms.length <= 1
  visited = Set.new
  traverse = ->(room) do
    return if visited.include? room
    visited << room
    neighbours = rooms[room]
    neighbours.each { |n| traverse.call n }
  end
  traverse.call 0
  return true if visited.length == rooms.length
  false
end

pp can_visit_all_rooms [[1], [2], [3], []]
pp can_visit_all_rooms [[1, 3], [3, 0, 1], [2], [0]]
